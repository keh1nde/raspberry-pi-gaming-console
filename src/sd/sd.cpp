 ...
 *  Index error, per the standing "identification first stage"
 *
...
/**
 * @brief Brings up the SD card and leaves it ready for sdio_read()/
 *  sdio_write() — clock setup, card identification (CID/RCA), and card
 *  selection all happen here.
 *
 * Call once at boot before touching the card; this is what
 * disk_initialize() (diskio.c) calls on FatFs's behalf. Safe to call
 * again later (e.g. after a card re-insertion) since it starts from a
 * full Software Reset each time.
 *
 * @return true if a card is present and ready to read/write; false
 *  otherwise (no card, or an unusable/incompatible card) — check the
 *  UART log for the specific reason.
 */
bool sdio_init() {
 _sdio_clock_init();
...
/**
 * @brief Reads `count` 512-byte blocks starting at LBA `lba` into `buf`.
 *
 * `sdio_init()` must have already succeeded. Uses CMD17 for a single
 * block, CMD18 (with Auto CMD12) for multiple.
 *
 * @param lba Starting logical block address (0-indexed, 512-byte blocks).
 * @param count Number of blocks to read; must be at least 1.
 * @param buf Destination buffer, must be at least `count * 512` bytes.
 * @return true on success; false on any bus, data, or card-level error
 *  (logged over UART with the specific cause).
 */
...
/**
 * @brief Writes `count` 512-byte blocks starting at LBA `lba` from `buf`.
 *
 * `sdio_init()` must have already succeeded. Uses CMD24 for a single
 * block, CMD25 (with Auto CMD12) for multiple.
 *
 * @param lba Starting logical block address (0-indexed, 512-byte blocks).
 * @param count Number of blocks to write; must be at least 1.
 * @param buf Source buffer, must contain at least `count * 512` bytes.
 * @return true on success; false on any bus, data, or card-level error
 *  (logged over UART with the specific cause).
 */
bool sdio_write(const uint32_t lba, const uint32_t count, const uint8_t* buf) {
 ...
 uint8_t* const rw_buf = const_cast<uint8_t*>(buf);
